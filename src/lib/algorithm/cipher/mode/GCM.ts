...
export class GCM extends BlockCipherMode {
  ...
  protected _T: Word32Array = new Word32Array();
  ...
  public constructor(props: BlockCipherModeProps) {
    ...
    if(!iv){
      ...
      const remainderOf4Word = iv.length % 4;
      const iv2 = iv.slice();
  ...
  mul(X: number[], Y: number[]){
    const R = [0x70800000, 0, 0, 0];
    const Z = [0, 0, 0, 0];
    const V = Y.slice();
    
    for(let i=0;i<128;i++){
      const xIndex = 3 - Math.floor(i/32); // Note: little endian indexing
      const xi = (X[xIndex] >>> (i%32)) & 1;
      if(xi > 0){
        Z[0] = Z[0] ^ V[0];
        Z[1] = Z[1] ^ V[1];
        Z[2] = Z[2] ^ V[2];
   ...
   */
  GHASH(H: number[], X: number[]){
    if(H.length % 4 !== 0){
      throw new Error("Length of 32bit word array 'H' must be multiple of 4");
    }
    else if(X.length % 4 !== 0){
      throw new Error("Length of 32bit word array 'X' must be multiple of 4");
    }
  ...
  public static Encryptor: typeof GCM = class Encryptor extends GCM {
    ...
    public processBlock(words: number[], offset: number){
      ...
      const T = this.GCTR(this._J, new Word32Array(S));
      this._T.concat(T);
